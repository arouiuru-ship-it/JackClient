    ...
    public void onInitializeClient() {
        ...
        ClientTickEvents.END_CLIENT_TICK.register(c -> {
            ...
            if (autoMine && c.world != null && c.interactionManager != null) {
                ...
                // 2. Ищем блок если нет дропа
                if (targetCenter == null) {
                    if (autoMineTarget == null || !blockMatches(c.world.getBlockState(autoMineTarget))
                        || c.world.getBlockState(autoMineTarget).isAir()) {
                        autoMineTarget = findAutoMineBlock(c);
                    }
                    if (autoMineTarget != null) {
                        targetCenter = net.minecraft.util.math.Vec3d.ofCenter(autoMineTarget);
                    }
                }
                ...
                // 5. Цель блок - рядом копаем
                if (fullDist < 3.5 && autoMineTarget != null) {
                    c.player.setVelocity(0, c.player.getVelocity().y, 0);
                ...
                // 6. Сканируем путь: ищем первое препятствие
                net.minecraft.util.math.BlockPos obstacle = null;
                if (horizDist > 0.1) {
                    double nx = dx / horizDist;
                    double nz = dz / horizDist;
                    for (double step = 1.0; step < horizDist && step < 12.0; step += 0.8) {
                        double cx = c.player.getX() + nx * step;
                        double cz = c.player.getZ() + nz * step;
                        // проверяем на уровне ног и головы
                        net.minecraft.util.math.BlockPos feet = new net.minecraft.util.math.BlockPos((int)Math.floor(cx), (int)Math.floor(c.player.getY()), (int)Math.floor(cz));
                        net.minecraft.util.math.BlockPos head = new net.minecraft.util.math.BlockPos((int)Math.floor(cx), (int)Math.floor(c.player.getY() + 1.0), (int)Math.floor(cz));
                        net.minecraft.block.BlockState stateFeet = c.world.getBlockState(feet);
                        net.minecraft.block.BlockState stateHead = c.world.getBlockState(head);
                        if (!stateFeet.isAir() && !(stateFeet.getBlock() instanceof net.minecraft.block.FluidBlock)) {
                            obstacle = feet; break;
                        }
                        if (!stateHead.isAir() && !(stateHead.getBlock() instanceof net.minecraft.block.FluidBlock)) {
                            obstacle = head; break;
                        }
                    }
                }
                ...
                if (obstacle != null) {
                    // 7. Есть препятствие - копаем ЕГО (не цель через стену)
                ...
                // 8. Путь свободен - идём
                if (horizDist > 0.5) {
                    double speed = 0.22;
                    c.player.setVelocity(dx/horizDist * speed, c.player.getVelocity().y, dz/horizDist * speed);

                    // Автопрыжок если блок высотой 1
